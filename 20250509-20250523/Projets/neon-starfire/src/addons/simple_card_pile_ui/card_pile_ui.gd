...
enum Piles { draw_pile, hand_pile, discard_pile }

enum PilesCardLayouts { up, left, right, down }
...
var _hand_pile: Array[CardUI] = []  # an array of `CardUI`s
...
func _create_card_ui(json_data: Dictionary):
	...
	for key in json_data.keys():
		if (
			key != "texture_path"
			and key != "backface_texture_path"
			and key != "resource_script_path"
		):
